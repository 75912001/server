...
func validateStoredCharacterMail(fieldUUID uint64, usedUUID uint64, mailRecord *pb.MailRecord) error {
	if fieldUUID == 0 || mailRecord.GetUuid() != fieldUUID || fieldUUID > usedUUID {
		return errors.Wrapf(errCharacterMailboxDataInvalid, "mail uuid mismatch, field: %d, record: %d, used: %d", fieldUUID, mailRecord.GetUuid(), usedUUID)
	}
	title, content, err := common.NormalizeSystemMailText(mailRecord.GetTitle(), mailRecord.GetContent())
	if err != nil || title != mailRecord.GetTitle() || content != mailRecord.GetContent() {
		return errors.Wrapf(errCharacterMailboxDataInvalid, "mail text is invalid, uuid: %d", fieldUUID)
	}
	createTimestampMs := mailRecord.GetCreateTimestampMs()
	expireTimestampMs := mailRecord.GetExpireTimestampMs()
	if createTimestampMs <= 0 ||
		expireTimestampMs <= createTimestampMs ||
		expireTimestampMs-createTimestampMs != characterMailExpireDuration().Milliseconds() {
		return errors.Wrapf(errCharacterMailboxDataInvalid, "mail timestamp is invalid, uuid: %d", fieldUUID)
	}
	return nil
}

func decodeCharacterMailboxHash(values map[string]string, nowTimestampMs int64) (*pb.MailboxRecord, []string, error) {
	mailboxRecord := &pb.MailboxRecord{MailRecordMap: make(map[uint64]*pb.MailRecord)}
	if len(values) == 0 {
		return mailboxRecord, nil, nil
	}

	usedUUIDText, ok := values[characterMailboxFieldUsedUUID]
	if !ok {
		return nil, nil, errors.Wrap(errCharacterMailboxDataInvalid, "usedUUID field is missing")
	}
	usedUUID, err := strconv.ParseUint(usedUUIDText, 10, 64)
	if err != nil || usedUUID == 0 || strconv.FormatUint(usedUUID, 10) != usedUUIDText {
		return nil, nil, errors.Wrapf(errCharacterMailboxDataInvalid, "usedUUID field is invalid: %q", usedUUIDText)
	}
	...
	for field, value := range values {
		...
		if !strings.HasPrefix(field, characterMailboxFieldPrefix) {
			return nil, nil, errors.Wrapf(errCharacterMailboxDataInvalid, "unknown field: %q", field)
		}
		mailUUIDText := strings.TrimPrefix(field, characterMailboxFieldPrefix)
		mailUUID, err := strconv.ParseUint(mailUUIDText, 10, 64)
		if err != nil || mailUUID == 0 || strconv.FormatUint(mailUUID, 10) != mailUUIDText {
			return nil, nil, errors.Wrapf(errCharacterMailboxDataInvalid, "mail field is invalid: %q", field)
		}

		mailRecord := &pb.MailRecord{}
		if err := proto.Unmarshal([]byte(value), mailRecord); err != nil {
			return nil, nil, errors.Wrapf(errCharacterMailboxDataInvalid, "unmarshal mail failed, uuid: %d", mailUUID)
		}
		if err := validateStoredCharacterMail(mailUUID, usedUUID, mailRecord); err != nil {
			return nil, nil, err
		}
	...
	return mailboxRecord, expiredFields, nil
}
...
func (p *Redis) GetCharacterMailbox(ctx context.Context, aid uint64, characterUUID uint64) (*pb.MailboxRecord, error) {
	...
	mailboxRecord, expiredFields, err := decodeCharacterMailboxHash(values, time.Now().UnixMilli())
	if err != nil {
		return nil, err
	}
...
func (p *Redis) characterMailboxMailCount(ctx context.Context, aid uint64, characterUUID uint64) (int64, error) {
	...
	usedUUIDExists, err := p.client.HExists(ctx, key, characterMailboxFieldUsedUUID).Result()
	if err != nil {
		return 0, errors.WithMessagef(err, "check character mailbox usedUUID failed, aid: %d, characterUUID: %d %v", aid, characterUUID, xruntime.Location())
	}
	if !usedUUIDExists {
		return 0, errors.Wrap(errCharacterMailboxDataInvalid, "usedUUID field is missing")
	}
...
func (p *Redis) nextCharacterMailUUID(ctx context.Context, aid uint64, characterUUID uint64) (uint64, error) {
	key := RedisKeyCharacterMailbox(aid, characterUUID)
	usedUUIDText, err := p.client.HGet(ctx, key, characterMailboxFieldUsedUUID).Result()
	if err != nil && !errors.Is(err, redis.Nil) {
		return 0, errors.WithMessagef(err, "get character mailbox usedUUID failed, aid: %d, characterUUID: %d %v", aid, characterUUID, xruntime.Location())
	}
	if err == nil {
		usedUUID, parseErr := strconv.ParseUint(usedUUIDText, 10, 64)
		if parseErr != nil || usedUUID == 0 || strconv.FormatUint(usedUUID, 10) != usedUUIDText {
			return 0, errors.Wrapf(errCharacterMailboxDataInvalid, "usedUUID field is invalid: %q", usedUUIDText)
		}
		if usedUUID >= math.MaxInt64 {
...
func (p *Redis) getCharacterMail(ctx context.Context, aid uint64, characterUUID uint64, mailUUID uint64) (*pb.MailRecord, error) {
	...
	usedUUIDText, err := p.client.HGet(ctx, key, characterMailboxFieldUsedUUID).Result()
	if err != nil {
		return nil, errors.WithMessagef(errCharacterMailboxDataInvalid, "get usedUUID for character mail failed, aid: %d, characterUUID: %d, mailUUID: %d: %v", aid, characterUUID, mailUUID, err)
	}
	usedUUID, err := strconv.ParseUint(usedUUIDText, 10, 64)
	if err != nil || usedUUID == 0 || strconv.FormatUint(usedUUID, 10) != usedUUIDText {
