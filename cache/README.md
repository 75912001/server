...
- `PetRecord.asset_id` 直接保存宠物资源 ID/宠物配置 ID; `PetRecord.nick` 只保存用户自定义昵称, 空字符串表示客户端显示宠物配置名称; `PetRecord.exp/loyalty/saved_base_*/raw_*/create_timestamp_ms/rebirth_count` 直接保存宠物经验、忠诚度、成长基础值、当前原始属性、创建时间和转生次数; `asset_record_base_map` 不承载宠物资源 ID。
