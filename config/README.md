...
- `item.yaml`: 道具和装备配置, 使用 `items.<group>.<id>` 分组; 非零 `sprite` 必须同时配置以 `item/` 开头的无扩展名 `atlas` 路径, 武器ID必须位于对应协议分组区间. 武器条目可保存原版名称、说明、价格、装备等级、职业限制、套装编号、攻击次数、能力随机范围、元素和异常抗性; 未配置的数值字段默认为0, 非法范围会导致启动失败.
