...
Login 服务负责对外提供 HTTP 登录入口, 完成 accountVerifyToken 的写入和消费, 并为客户端分配可用 gateway。部署、端口、容器启动和验证命令见 `deploy/login/README.md`。

## 能力边界

- 提供 `POST /api/login/accountVerifyToken`, 接收外部程序提交的 `account` 和 `accountVerifyToken`, 通过 cache 写入 accountVerifyToken。
- 提供 `POST /api/login/session`, 接收客户端提交的 `account` 和 `accountVerifyToken`, 通过 cache 验证并消费 accountVerifyToken, 获得可信 uid。
...
1. 校验 HTTP 方法、JSON 结构、`account` 和 `accountVerifyToken`, 并去除 `account` 首尾空白。
2. 调用 cache `CacheSetAccountVerifyToken`。
3. cache 使用 `account:{account}:accountVerifyToken` 执行 `SETNX`, 未消费 accountVerifyToken 存在时返回失败。
4. 成功返回 `account`、`accountVerifyToken` 和 `expireSecond`。

失败语义：

- `400`：请求结构或参数错误。
- `409`: 同账号 accountVerifyToken 已存在且未消费。
...
1. 校验 HTTP 方法、JSON 结构、`account` 和 `accountVerifyToken`, 并去除 `account` 首尾空白。
2. 调用 cache `CacheUseAccountVerifyToken`, 校验成功即消费 accountVerifyToken。
3. 从 cache 返回可信 uid。
...
- accountVerifyToken 在 `CacheUseAccountVerifyToken` 校验成功后即标记为 used; 如果后续账号创建、gateway 选择或票据签发失败, 客户端需要重新申请 accountVerifyToken。
...
```text
外部程序
  -> login /api/login/accountVerifyToken
...
- `accountVerifyToken not found or used`: accountVerifyToken 不存在、已消费、已过期或值不匹配。
...
- 为 `POST /api/login/session` 增加覆盖 accountVerifyToken 重放、ticket 过期/篡改、gateway 不可用的测试。
