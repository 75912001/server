...
Login 服务是账号登录链路的 HTTP 入口，负责接收外部程序写入的 accountVerifyToken，并在客户端登录时消费 accountVerifyToken、选择 gateway、签发短期 `connectTicket`。部署、端口、容器启动和验证命令见 `deploy/login/README.md`。

## 服务设计

login 只做 accountVerifyToken 和 gateway 入口分配，不维护用户在线态。
