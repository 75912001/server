...
- `AccountRecord` 由 online 登录绑定时从 cache 读取, online 业务更新时再写回 cache。cache 建号时创建账号基础字段、固定 5 个空角色槽位和空宠物仓库; 具体角色、宠物和后续业务数据由 online 填充和维护。
