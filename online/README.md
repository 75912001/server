...
`scene/*.yaml` 不设置格式版本字段, 地图 ID 与客户端 `map_id` 完全一致. 当前目录包含80000、80001、80010、80020、80030、80040这6张测试地图, 以及从90001开始的14张练级地图: 90001和90010至90130按10递增. 角色正常进入接受协议测试地图范围`[80000,89999]`和练级地图范围`[90000,99999]`. 每张地图通过平铺的 `encounter.enabled` 和 `encounter.enemyGroups` 定义全地图遇敌规则, 自动遇敌不接收或读取角色坐标. 自动遇敌通过 `scene/*.yaml -> enemy.group.yaml` 取得敌人条目, 再分别读取宠物模板与 AI 生成单位; 服务端按权重选择敌人组, 敌人组的 `enemies[].id` 直接引用宠物模板, 数量和等级来自敌人组, 属性来自宠物模板, AI 由敌人条目的 `battleAI` ID 显式引用, 经验由 `enemy.exp.yaml` 结合宠物模板计算. 每个敌人的技能、权重和目标策略均来自 `ai.yaml`, 建房时深拷贝到服务端战斗运行态, 不下发客户端.
...
`TaskAcceptReq/Res`、`TaskSubmitReq/Res`、`TaskStepRewardClaimReq/Res`和`CharacterTaskChangedNotify`使用消息范围`0x007000-0x007006`. 接取、提交和领取只允许当前账号内已上线且不在战斗中的角色. 请求只携带角色、任务和必要的步骤ID, 不接收客户端自报进度、条件结果或奖励内容.
...
任务管理器在候选角色档案上校验条件并推进串行步骤. 提交先核对全部完成条件和扣除数量, 奖励通过统一道具管理器发放; 任务记录、道具扣除/增加与领取标记共用一次账号持久化. 任一校验、容量、数量溢出或Cache失败均不得提交部分状态. 步骤奖励在该步骤完成后即可领取, 不要求整个任务结束, 也不阻塞后续步骤; 无奖励步骤在完成时直接把领取时间设为完成时间.

成功持久化后发送道具最终数量和任务变化通知, 再回复操作成功. `CharacterTaskChangedNotify.changed_task_record_map`仅包含本次实际变化的任务, 每个变化任务携带完整步骤记录, 未变化任务不下发. 客户端用该增量按ID合并, 可接任务和页签状态由客户端配置与已有记录推导.
...
任务记录绑定时校验任务存在、步骤数量与ID、时间戳顺序、串行状态和无奖励已领取标记. V1不迁移旧任务档案, 不支持重接、放弃、循环任务或离线补发奖励. 阿布水购买资格以后直接读取任务60是否完成, 购买入口尚待接入.
...
`ShopPurchaseReq` 购买 `item.weapon.yaml` 中 `cost > 0` 的武器, `cost` 当前固定使用 `AssetID_Stone` 计价. 商店没有 NPC、场景、库存或限购条件; 服务端按当前配置价格结算, 并校验角色属于当前账号、已经在线且不在战斗中, 以及数量、石币资产、背包剩余格数和账号 UUID 游标. `cost = 0`、非武器 ID 或数量非法都必须拒绝.
...
换装在 Account actor 内串行构造完整账号候选档案. 装备或替换时从背包移出新武器, 并把旧武器放回背包; 卸下时要求背包仍有空位. cache 持久化成功后才同时提交角色背包、穿戴记录和运行中角色引用, 失败时权威档案不发生任何变化. 成功后刷新场景 Presence 中的武器类型, 并通过统一场景表现变化通知把 `weapon_type` 广播给当前可见的其他角色; 换装角色本人使用成功响应返回的完整 `item_bag`、本次替换部位的 `equipment` 和服务器重算后的 `effective_attribute` 原子更新本地档案, 卸下时 `equipment` 不设置.
