...
func newUser(remote xnetcommon.IRemote, shard *UserShard) *User {
	u := &User{remote: remote, shard: shard}
	u.startVerifyTimer()
	return u
}
...
func (u *User) startVerifyTimer() {
	cb := xcontrol.NewCallBack(func(args ...any) error {
		...
		xlog.PrintInfo(fmt.Sprintf("user[%s] verify timeout, disconnect", u.remote.GetIP()))
		u.remote.Stop()
		return nil
	})
...
func (u *User) OnHeartbeatReq(header *xpacket.Header, body []byte) error {
	...
	if !u.verifyd.Load() {
		xlog.PrintfErr("heartbeat before verify, remote=%s", u.remote.GetIP())
		u.remote.Stop()
		return nil
	}
