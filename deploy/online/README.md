...
online 启动时会先加载共享游戏配置. 镜像构建会把仓库 `config/` 复制到 `/app/config`, `deploy/online/*.yaml` 使用 `custom.gameConfigDir: /app/config`. server 只校验服务端消费字段和跨表引用; 角色展示字段, 宠物展示字段, 客户端 PNG, `.tpsheet` 和 frame 资源仍由 sa.desktop 校验.
